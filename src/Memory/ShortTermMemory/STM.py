...
class STM:
    ...
    def __init__(self, config: Dict[Any, object], memory_cache: MemoryCache):
        ...
        self.maxstepsSize = max(int(self.config.get("maxsteps", 1000)), 1)
    ...
    def _loaddb(self):
        ...
        try:
            ...
            if not isinstance(database, list):
                ...
                logger.warning("Invalid STM database format")

                return []

            database = [event for event in database if isinstance(event, MemoryEvent)]

            if len(database) > self.maxstepsSize:

                database = database[-self.maxstepsSize :]
    ...
    def add(self, event: MemoryEvent):
        ...
        if not self.memorycache.add(event):

            logger.error("Failed to add event to MemoryCache")
    ...
    def flushEverything(self):
        ...
        cache_flushed = self.memorycache.flush()

        if not cache_flushed:

            logger.error("Cannot flush MemoryCache")
