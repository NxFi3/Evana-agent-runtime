...
class Retrieval:
    ...
    def _load_or_build_index(self):
        ...
        try:
            ...
            if loaded_index.d != self.dimension:
                ...
                logger.warning("FAISS dimension mismatch. Rebuilding index.")
            ...
            logger.info(f"FAISS loaded successfully. " f"Vectors: {self.index.ntotal}")
        ...
        try:
            ...
            for memory_id, embedding in records:
                ...
                if embedding.shape[1] != self.dimension:
                    ...
                    logger.warning(
                        f"Skipping ID {memory_id}: " f"wrong embedding dimension."
                    )
            ...
            if not self._save_index():
                raise RuntimeError("Failed to save rebuilt FAISS index.")

            logger.info(f"FAISS index built. " f"Vectors: {self.index.ntotal}")
    ...
    def add(self, memory_id: int, embedding: np.ndarray) -> bool:

        if embedding is None:

            logger.warning(f"Cannot add memory {memory_id}: " f"embedding is None.")
        ...
        try:
            ...
            memory_id_array = np.asarray([int(memory_id)], dtype=np.int64)

            self.index.remove_ids(memory_id_array)

            self.index.add_with_ids(embedding, memory_id_array)
            ...
            logger.error(f"Failed to add memory " f"{memory_id}: {e}")
    ...
    def remove(self, memory_id: int) -> bool:
        ...
        try:
            ...
            removed = self.index.remove_ids(
                np.asarray([int(memory_id)], dtype=np.int64)
            )
            ...
            logger.error(f"Failed to remove memory " f"{memory_id} from FAISS: {e}")

            return False

    def _dense_search(
        self, query_embedding: np.ndarray, limit: int = 100
    ) -> List[Dict[str, Any]]:
        ...
        limit = min(int(limit), self.index.ntotal)
        ...
        for rank, (memory_id, distance) in enumerate(
            ...
            results.append(
                {"id": int(memory_id), "rank": rank, "distance": float(distance)}
            )

        return results

    def get_nearest_memory(self, query: str) -> Optional[Dict[str, Any]]:
        ...
        query_embedding = self.EmbeddingModel.encode(f"query: {query}")
        ...
        return {"id": memory_id, "similarity": float(distances[0][0])}
    ...
    def _compute_rrf(
        ...
        for result in dense_results:
            ...
            dense_score = 1.0 / (k_rrf + result["rank"])
        ...
        if dense_only:
            ...
            ids = [result["id"] for result in dense_only]
            ...
            for result in dense_only:
                ...
                merged[memory_id] = {
                    "item": item,
                    "rrf_score": 1.0 / (k_rrf + result["rank"]),
                    "bm25_rank": None,
    ...
    def Retrieve(self, query: str, top_k: int = 10) -> List[Dict[str, Any]]:
        ...
        bm25_results = self.db.search_fts(query, limit=100)

        dense_results = self._dense_search(query_embedding, limit=100)

        if not bm25_results and not dense_results:
            return []

        rrf_results = self._compute_rrf(bm25_results, dense_results, k_rrf=60, limit=20)

        if not rrf_results:
            return []

        documents = [result["item"].value for result in rrf_results]

        reranker_scores = self.Reranker.rank(query=query, documents=documents)

        if len(reranker_scores) != len(rrf_results):

            raise RuntimeError("Reranker returned an invalid " "number of scores.")

        for result, score in zip(rrf_results, reranker_scores):

            result["reranker_score"] = float(score)

        rrf_results.sort(key=lambda x: x["reranker_score"], reverse=True)
