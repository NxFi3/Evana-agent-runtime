...
class DBManager:
    ...
    def __init__(self, db_path: str = "") -> None:
        ...
        self.database_path = db_path or "Long_Term_Memory.db"
    ...
    def _sync_fts(self, cursor):
        ...
        logger.info("Synchronizing FTS5 index...")
    ...
    def add_item(
        ...
        if not value:
            logger.warning("Cannot add empty memory value")
            return None
        ...
        try:
            ...
            graph_blob = (
                np.asarray(graph, dtype=np.float32).tobytes()
    ...
    def get_by_id(
        self, item_id: int, embedding_dtype=np.float32, include_deleted: bool = False
    ) -> Optional[MemoryItem]:
        ...
        try:
            ...
            return self._row_to_memory_item(row, embedding_dtype)
    ...
    def get_by_ids(
        ...
        try:
            ...
            items = [self._row_to_memory_item(row, embedding_dtype) for row in rows]
    ...
    def search_fts(
        self, query: str, limit: int = 50, include_deleted: bool = False
    ) -> list[MemoryItem]:
        ...
        try:
            ...
            results = self._fts_query(cursor, fts_query, limit, include_deleted)

            if not results and len(safe_words) > 1:

                fts_query = " OR ".join(f'"{word}"' for word in safe_words)

                results = self._fts_query(cursor, fts_query, limit, include_deleted)

            return [self._row_to_memory_item(row, np.float32) for row in results]
    ...
    def _fts_query(self, cursor, fts_query: str, limit: int, include_deleted: bool):
    ...
    def update_item(
        ...
        try:
            ...
            if new_graph is not None:
                ...
                params.append(np.asarray(new_graph, dtype=np.float32).tobytes())
            ...
            if new_importance is not None:
                ...
                params.append(float(new_importance))

            if new_embedding is not None:

                updates.append("embedding = ?")

                params.append(np.asarray(new_embedding, dtype=np.float32).tobytes())

            if soft_delete is not None:

                updates.append("deleted = ?")

                params.append(1 if soft_delete else 0)
    ...
    def mark_accessed(self, item_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Mark accessed error: {e}")
    ...
    def delete_item(self, item_id: int, hard_delete: bool = False) -> bool:
        ...
        if hard_delete:
            ...
            try:
                ...
                logger.debug(f"Hard deleted memory item: {item_id}")
        ...
        return self.update_item(item_id, soft_delete=True)

    def restore_by_id(self, item_id: int) -> bool:
    ...
    def get_all_embeddings(self):
        ...
        try:
            ...
            for memory_id, embedding_blob in cursor.fetchall():
                ...
                results.append((int(memory_id), embedding))
    ...
    def database_summary(self) -> dict:
        ...
        try:
            ...
            items_with_embedding = cursor.fetchone()[0]
    ...
    @staticmethod
    def _row_to_memory_item(row, embedding_dtype=np.float32) -> MemoryItem:
        ...
        embedding = (
            np.frombuffer(row[4], dtype=embedding_dtype).copy()
            if row[4] is not None
            else np.array([], dtype=embedding_dtype)
        )

        raw_score = row[10] if len(row) > 10 else None
