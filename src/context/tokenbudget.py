...
class TokenBudget:
    def __init__(self, config: Dict[str, Any], llm_provider: LlmProvider) -> None:
        ...
        context_length = self.llm_provider.model.defaultConfig.get("num_ctx", 120000)
        ...
        self.chars_per_token = float(self.config.get("initial_chars_per_token", 4.0))

        self.calibration_alpha = float(self.config.get("calibration_alpha", 0.2))

        # Prevent unstable calibration.
        self.min_chars_per_token = float(self.config.get("min_chars_per_token", 1.5))

        self.max_chars_per_token = float(self.config.get("max_chars_per_token", 8.0))
    ...
    def _message_character_count(self, message: Dict[str, Any]) -> int:
    ...
    def calibrate_from_response(
        self, messages: List[Dict[str, Any]], model_response: LLMResult
    ) -> None:
        ...
        actual_prompt_tokens = getattr(raw, "prompt_eval_count", 0) or 0
        ...
        observed_chars_per_token = measured_chars / actual_prompt_tokens

        observed_chars_per_token = max(
            self.min_chars_per_token,
            min(self.max_chars_per_token, observed_chars_per_token),
        )

        alpha = max(0.01, min(1.0, self.calibration_alpha))

        old_ratio = self.chars_per_token

        self.chars_per_token = (
            1.0 - alpha
        ) * old_ratio + alpha * observed_chars_per_token
