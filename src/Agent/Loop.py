...
class Loop:
    ...
    def _create_event(self,event_type:str,content:str,source:str,metadata:dict[str,object]|None=None)->MemoryEvent:
        self.steps+=1
        return MemoryEvent(event_type=event_type,content=content,source=source,step=self.steps,metadata=metadata or {})
    ...
    def Process(self,user_input:MemoryEvent,image:np.ndarray=None):
        ...
        for iteration in range(1,max_iterations+1):
            ...
            successful_tools=sum(
                1 for result in execution.get("results",[])
